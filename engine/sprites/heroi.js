...
Heroi.prototype = {
  atualizar: function() {
    ...
    // Movimentos
    if (this.teclado.pressionada(37)) {
    ...
    } else if (this.teclado.pressionada(39)) {
      ...
      this.imgPosX += this.velocidade;
    } else if (this.teclado.pressionada(38)) {
    ...
    } else if (this.teclado.pressionada(40)) {
  ...
  colidiuCom: function(sprite) {
    if (sprite.ident != "tiro") {
      if (sprite.x < this.imgPosX) {
        //Objeto colisor vem da esquerda pra direita ( Bola )
        sprite.velocidadeX = -Math.abs(sprite.velocidadeX); // +
      } else {
        //Objeto colisor vem da direita pra esquerda ( Bola )
        sprite.velocidadeX = Math.abs(sprite.velocidadeX); // +
      }

      if (sprite.y < this.imgPosY) {
        //Objeto colisor vem de baixo pra cima ( Bola )
        sprite.velocidadeY = -Math.abs(sprite.velocidadeY); // -
      } else {
        //Objeto colisor vem de cima pra baixo ( Bola )
        sprite.velocidadeY = Math.abs(sprite.velocidadeY); // +
      }
    }
  }
